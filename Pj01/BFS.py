class BFS:
    ...
    def ares_find_pos_bfs(self, ares_pos, target_pos, block):
        open_list = [ares_pos]
        close_set = set()
        ...
        while open_list:
            ...
            for next_pos in self.get_neighbor(current):
                if self.hash_obj(next_pos) not in close_set and not self.pos_of_array(next_pos, block):
                    open_list.insert(0,next_pos)

        return {"result": False, "data": None}

    def stone_bfs(self, ares_start, stones_start, gols, index):
        ...
        while open_list:
            ...
            for next_pos in self.get_neighbor(current):
                ...
                ares = self.ares_find_pos_bfs(new_ares_start, sym, stones_start)
    ...
    def run(self):
        ...
        for i in range(len(self.stones_start)):
            path = self.stone_bfs(start, self.stones_start, self.gol, i)
